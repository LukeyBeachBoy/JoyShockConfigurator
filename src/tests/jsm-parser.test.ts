...
import { ButtonBinding } from '../lib/button-binding';
...
describe('📤 ButtonBinding objects can be outputted into JSM format', () => {
    ...
    test('Hold press is correctly formatted and has different command to simple press', () => {
        const pressWithHoldBinding = new ButtonBinding({ button: Buttons.N, press: '1', hold: '2' });

        expect(pressWithHoldBinding.toJSM()).toBe('N = 1 2');
    });
});
